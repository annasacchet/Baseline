"""Shared constants for the Qwen3-30B-A3B rewriting + eval pipelines.

Qwen3-30B-A3B-Instruct-2507 is an MoE model (~30B total, ~3B active). It is
open-weights and runs on Lisa via vLLM (TP across the node's GPUs). This is
the "non-thinking" Instruct checkpoint, so no thinking-block stripping is
needed.
...
unchanged. The value below is Qwen, not OLMo.
"""

# Single model id used everywhere — Qwen3-30B-A3B-Instruct-2507.
# vLLM handles rewriting + Answer F1. Perplexity uses HF transformers
# directly (vLLM doesn't expose per-token log-likelihoods).
OLMO_MODEL_ID = "Qwen/Qwen3-30B-A3B-Instruct-2507"

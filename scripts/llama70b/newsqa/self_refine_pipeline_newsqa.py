"""NewsQA Self-Refine pipeline (RQ3) — Llama-3.1-70B-Instruct-AWQ-INT4 via vLLM.

Mirrors scripts/llama70b/musique/self_refine_pipeline_musique.py but the source
text per chain is a NewsQA story. We use vLLM (AWQ-INT4, TP=2) for the 3x
generations per step — self-refine triples the per-step compute, and vLLM is
the only practical backend on 2x A6000 for that workload.

E0 comes from the baseline rewriting CSV when present, so the starting text
is byte-identical to RQ1. If the baseline CSV is missing a qid we rebuild E0
from the NewsQA dataset (story_text).
"""
...
# Reuse the same NewsQA loader / quality filter as the baseline rewriting pipeline.
...
DEFAULT_OUTPUT_CSV = REPO_ROOT / "results" / "llama70b" / "newsqa" / "self_refine_chains_newsqa.csv"
DEFAULT_BASELINE_CSV = REPO_ROOT / "results" / "llama70b" / "newsqa" / "rewriting_chains_newsqa.csv"


def load_e0_from_baseline(csv_path: Path) -> dict:
    """{qid: (question, gold_answer, aliases_joined, E0_text)} from step=0 rows."""
...
def self_refine_step_batched(
    ...
    """One self-refine iteration over all `specs` of a single story."""
    rewriter_prompts = [
        render_chat(
            tokenizer, None,
...
def main():
    p = argparse.ArgumentParser(description="NewsQA self-refine chains with Llama-3.1-70B-AWQ via vLLM.")
    ...
    # See the rationale in scripts/llama70b/musique/self_refine_pipeline_musique.py.

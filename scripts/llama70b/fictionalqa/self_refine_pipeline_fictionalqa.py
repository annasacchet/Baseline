"""FictionalQA Self-Refine pipeline (RQ3) — Llama-3.1-70B-Instruct-AWQ via vLLM.

Mirrors scripts/llama70b/musique/self_refine_pipeline_musique.py but the
source text per chain is a FictionalQA document. vLLM (AWQ-INT4, TP=2) is
used for the 3x generations per step.

E0 is sourced from the baseline rewriting CSV when available (byte-identical
to RQ1); otherwise rebuilt from the HF dataset via the loader in the baseline
pipeline.
"""
...
DEFAULT_OUTPUT_CSV = REPO_ROOT / "results" / "llama70b" / "fictionalqa" / "self_refine_chains_fictionalqa.csv"
DEFAULT_BASELINE_CSV = REPO_ROOT / "results" / "llama70b" / "fictionalqa" / "rewriting_chains_fictionalqa.csv"


def load_e0_from_baseline(csv_path: Path) -> dict:
    """{qid: (question, gold_answer, aliases_joined, style, event_id, E0_text)}."""
...
def main():
    p = argparse.ArgumentParser(description="FictionalQA self-refine chains with Llama-3.1-70B-AWQ via vLLM.")

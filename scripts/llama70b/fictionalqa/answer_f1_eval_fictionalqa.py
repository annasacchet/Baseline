"""Answer F1 on FictionalQA chains — QA model = Llama-3.1-70B via vLLM (AWQ INT4).

Uses the MuSiQue-style "short answer" prompt: FictionalQA answers are short
natural-language answers (rather than verbatim spans), so we ask for a few
words rather than a strict verbatim copy.
"""
...
DEFAULT_CHAINS_CSV = REPO_ROOT / "results" / "llama70b" / "fictionalqa" / "rewriting_chains_fictionalqa.csv"
...
def main():
    ap = argparse.ArgumentParser(description="Answer F1 on FictionalQA chains, Llama-3.1-70B QA via vLLM (AWQ INT4).")

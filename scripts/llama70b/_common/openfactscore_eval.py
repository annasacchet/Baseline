"""OpenFActScore (source-faithfulness) — Llama-70B rewriting chains.
...
What we replaced vs the OLMo pipeline
-------------------------------------
The user's directive was "Llama replaces OLMo-3.1-32B-Instruct". OLMo-3.1-32B
was used as: rewriter, QA model, PPL model. It was NOT used inside OFS.

OFS uses two other models that we intentionally keep:
...
If you want to swap AFG to Llama-70B too, pass --afg-model meta-llama/Llama-3.1-70B-Instruct
and --use-4bit.
...
from _common.llama_constants import AFV_MODEL_ID, CHAIN_KEYS  # noqa: E402
...
def main():
    ap = argparse.ArgumentParser(description="OpenFActScore on Llama-70B rewriting chains.")
    ...
    ap.add_argument("--use-4bit", action="store_true",
                    help="Enable 4-bit NF4 for AFG/AFV (needed if AFG=Llama-70B).")
    args = ap.parse_args()

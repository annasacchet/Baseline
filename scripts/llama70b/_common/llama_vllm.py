"""vLLM helper for the Llama-3.1-70B-Instruct-AWQ pipeline.
...
faster than HF transformers + bnb 4-bit on a 70B model on 2x consumer GPUs.

What we get from this module:
  - load_vllm(model_id, ...) returns a vllm.LLM
  - chat_complete(llm, tokenizer, messages_list, sampling)
        Apply the tokenizer chat template to each conversation, run
        llm.generate in a single batch, return the decoded continuations
        in the same order.
...
* `quantization="awq_marlin"` autoselects the Marlin kernel which is the
  fastest INT4 path on Ampere/Ada (RTX A6000 = Ampere, RTX 6000 Ada = Ada).
* On a 2x 48 GB Homer node, tensor_parallel_size=2 splits each layer's weights
  across both GPUs (so each GPU holds ~20 GB of AWQ weights + KV cache).
"""
...
def load_vllm(
    ...
    *,
    ...
    max_model_len : max total tokens (prompt + completion) per request. Llama-3.1
        supports 128k but we don't need that — 8k is enough for MuSiQue chains
        and keeps the KV-cache memory bounded.
    quantization : 'awq_marlin' (fastest INT4 path), 'awq' (legacy), or None.
    """
    ...
    print(f"Loading {model_id} with vLLM (tp={tensor_parallel_size}, "
          f"quant={quantization}, max_len={max_model_len}) ...", flush=True)
    t0 = time.time()
    llm = LLM(
        model=model_id,
...
def generate_batch_vllm(
    ...
    # Preserve input order. vLLM's outputs match the input list order.

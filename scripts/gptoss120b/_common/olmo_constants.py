"""Shared constants for the gpt-oss-120b rewriting + eval pipelines.

openai/gpt-oss-120b is an open-weights MoE model (~120B total, ~5B active),
served on Homer via vLLM (TP across the node's GPUs). The forward eval
(rewriting / Answer F1 / BERTScore / BLEURT / perplexity / token length)
runs on Homer; OpenFActScore + recall are run separately on Lisa.
...
NOTE: the module is named `olmo_constants` (and the symbol `OLMO_MODEL_ID`)
purely so the cloned import paths from the olmo32b pipeline keep working
unchanged. The value below is gpt-oss-120b, not OLMo.
"""

# Single model id used everywhere — openai/gpt-oss-120b.
# vLLM handles rewriting + Answer F1. Perplexity uses HF transformers
# directly (vLLM doesn't expose per-token log-likelihoods).
OLMO_MODEL_ID = "openai/gpt-oss-120b"

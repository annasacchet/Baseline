...
Mirrors scripts/llama70b/_common/bertscore_eval.py. Use --input / --output to
point at any of the OLMo-32B chain CSVs (musique / newsqa / fictionalqa). The
text pairs are built directly from CHAIN_KEYS + step columns; the rewriter
model is irrelevant here because BERTScore uses roberta-large internally.
...
def main():
    ap = argparse.ArgumentParser(description="BERTScore on OLMo-32B rewriting chains.")

"""MuSiQue Self-Refine pipeline (RQ3) — OLMo-3.1-32B-Instruct (bf16) via vLLM.

Generates rewriting chains E_0 → Ẽ_1 → Ẽ_2 → Ẽ_3 using a Rewriter / Critic /
Refiner loop where all three roles are the same OLMo-3.1-32B model.
...
Self-refine triples per-step compute. vLLM with TP=2 in bf16 keeps both
GPUs busy by batching all 12 chains of a question through each phase.
...
DEFAULT_OUTPUT_CSV = REPO_ROOT / "results" / "gptoss120b" / "musique" / "self_refine_chains_musique.csv"
DEFAULT_BASELINE_CSV = REPO_ROOT / "results" / "gptoss120b" / "musique" / "rewriting_chains_musique.csv"
...
def main():
    p = argparse.ArgumentParser(description="MuSiQue self-refine chains with OLMo-3.1-32B via vLLM.")
    ...
    if needs_rebuild:
        if not args.dataset.exists():
            print(f"ERROR: dataset not found ({args.dataset}) but {len(needs_rebuild)} qids "
                  f"are missing from baseline CSV.", file=sys.stderr); sys.exit(1)
    ...
    print(f"Plan: {len(qids_to_run)} questions x 4 x {n_runs_effective} = {total_chains} chains, "

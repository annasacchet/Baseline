"""FictionalQA rewriting chain pipeline — OLMo-3.1-32B-Instruct (bf16) via vLLM.

Same structure as scripts/fictionalqa/rewriting_pipeline_fictionalqa.py but the
rewriter is OLMo-3.1-32B-Instruct served by vLLM with tensor parallelism, and
the prompt uses the validated XML + system-prompt fix.
...
DEFAULT_OUTPUT_CSV = REPO_ROOT / "results" / "gptoss120b" / "fictionalqa" / "rewriting_chains_fictionalqa.csv"
...
def main():
    p = argparse.ArgumentParser(description="FictionalQA rewriting chains with OLMo-3.1-32B via vLLM.")

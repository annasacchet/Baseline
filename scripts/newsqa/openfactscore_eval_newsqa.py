"""
OpenFActScore (source-faithfulness) for NewsQA rewriting chains.

Identical pipeline to openfactscore_eval.py used for MuSiQue, adapted for
the NewsQA CSV schema.

Key differences from the MuSiQue version
----------------------------------------
- The NewsQA chains CSV already carries question + gold_answer + aliases — no
  separate dataset file is needed for AFV.
- The AFV "topic" string is derived from the article itself (first non-empty
  line of E_0, truncated). NewsQA storyIds are filesystem-style paths
  ("./cnn/stories/abc123.story") and would be useless as topics.
...
Source for AFV is E_0 (the step=0 story text for each chain).

Output
------
  rewriting_chains_newsqa_openfactscore.csv    — one row per (chain, step)
  rewriting_chains_newsqa_openfactscore_details.csv — one row per (chain, step, fact)
"""
...
DEFAULT_CSV = REPO_ROOT / "results" / "newsqa" / "rewriting_chains_newsqa.csv"
...
def derive_topic(text: str, fallback: str) -> str:
    """Pick a short, semantically meaningful topic string from the article.

    NewsQA storyIds look like "./cnn/stories/abc123.story" — useless as topics.
    Take the first non-empty line of E_0 (the article lead/headline) and trim
    it. Fall back to the qid only if no usable line exists.
    """
...
def main():
    parser = argparse.ArgumentParser(description="OpenFActScore over NewsQA rewriting chains.")
    ...
    print("=" * 70)
    print("OpenFActScore — source-faithfulness against E_0 (NewsQA)")

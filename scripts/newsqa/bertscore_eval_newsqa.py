"""
BERTScore evaluation for NewsQA 100q rewriting chains.

Mirrors scripts/15q/bertscore_eval.py: computes BERTScore in two modes
  - Baseline:    sim(E_t, E_0)
  - Consecutive: sim(E_t, E_{t-1})

Model: roberta-large (layer 17), same as the 7-qid pilot.

Resumable: if --output already exists, only missing (chain, step) rows are
recomputed and merged in.
"""
...
DEFAULT_CSV = REPO_ROOT / "results" / "newsqa" / "rewriting_chains_newsqa_100q.csv"
DEFAULT_OUT = REPO_ROOT / "results" / "newsqa" / "rewriting_chains_newsqa_100q_bertscore.csv"
...
def main():
    parser = argparse.ArgumentParser(description="BERTScore (Baseline + Consecutive) on NewsQA 100q chain CSV.")
    parser.add_argument("--input", type=Path, default=DEFAULT_CSV)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--batch-size", type=int, default=32, help="BERTScore batch size (default: 32 — H100/A100 friendly).")
    ...
    print("=" * 70)
    print(f"BERTScore NewsQA 100q  —  device={device}, batch={args.batch_size}")

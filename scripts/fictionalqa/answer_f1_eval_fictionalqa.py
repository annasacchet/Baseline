"""
Answer F1 evaluation for FictionalQA rewriting chains.
...
steps, compute Answer F1 against the FictionalQA gold answers.

Why FictionalQA is special for this evaluation
----------------------------------------------
The questions are about FICTIONAL events the QA model has never seen during
pretraining (per Kirchenbauer et al. 2026). This means F1 here measures pure
information survival in the rewriting chain — the QA model cannot "cheat" by
falling back on prior knowledge. Compare with NewsQA / MuSiQue where CNN
articles / Wikipedia paragraphs are likely in the LLM's pretraining corpus.

Schema
------
The chains CSV contains:
  - question, gold_answer, gold_answer_aliases (||-joined natural + span answers)
F1 is computed as max over (gold + aliases), matching the convention used in
the NewsQA / MuSiQue pipelines.

E_0 is deduplicated across instruction_types of the same (qid, run) and then
broadcast back, matching the NewsQA version's behaviour.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Answer F1 evaluation on FictionalQA rewriting chains.")

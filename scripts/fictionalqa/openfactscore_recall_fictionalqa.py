"""
OpenFActScore — RECALL for FictionalQA.

Reuses the atomic facts already extracted from each E_0 fiction document by
the forward OpenFActScore pipeline (rows with label='E0' in the
rewriting_chains_fictionalqa_openfactscore_details.csv file). For each
...
Output:
  results/fictionalqa/rewriting_chains_fictionalqa_openfactscore_recall.csv
    qid, group, instruction_type, run, step, instruction_used,
    n_e0_facts, n_recalled, n_not_recalled, recall_init, recall

  results/fictionalqa/rewriting_chains_fictionalqa_openfactscore_recall_details.csv
...
DEFAULT_CHAIN_CSV = REPO_ROOT / "results" / "fictionalqa" / "rewriting_chains_fictionalqa.csv"
DEFAULT_DETAILS_CSV = (
    REPO_ROOT / "results" / "fictionalqa"
    / "rewriting_chains_fictionalqa_openfactscore_details.csv"
)
...
def derive_topic(text: str, fallback: str) -> str:
    """First non-empty line of E_0, trimmed — same heuristic as the forward
    fictional OFS, so recall AFV sees a consistent topic string."""
...
def main():
    parser = argparse.ArgumentParser(
        description="OFS Recall for FictionalQA — reuses E_0 facts from the forward OFS details CSV."
    )
    ...
    parser.add_argument("--limit", type=int, default=None,
                        help="Smoke-test: first N qids only.")
    parser.add_argument("--qid", action="append", default=None,
                        help="Process only these qids (repeatable).")
    ...
    print("=" * 70)
    print("OpenFActScore RECALL (FictionalQA) — AFV-only, reusing E_0 facts")
    ...
    e0_text_by_qid = {
        qid: grp.iloc[0]["text"]
        for qid, grp in chain[chain["step"] == 0].groupby("qid", sort=False)
    }
    ...
    for i, (_, row) in enumerate(to_eval.iterrows(), start=1):
        ...
        topic = derive_topic(e0_text_by_qid.get(row["qid"], ""), row["qid"])
        label_str = (
        ...
        result = compute_recall(afv, e0_facts, row["text"], topic, gamma=args.gamma)

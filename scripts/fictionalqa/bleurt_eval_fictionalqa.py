"""
BLEURT evaluation for the FictionalQA rewriting chains.
...
Note on FictionalQA: the gold answers are short factoid strings about fictional
events. bleurt_answer is especially meaningful here because the QA model cannot
rely on prior knowledge — any degradation is purely due to the rewriting.

Output CSV (one row per step > 0):
  qid, group, instruction_type, run, step,
  bleurt_baseline, bleurt_consecutive, bleurt_answer

Uso:
  python3 scripts/fictionalqa/bleurt_eval_fictionalqa.py
  python3 scripts/fictionalqa/bleurt_eval_fictionalqa.py --f1-csv results/fictionalqa/rewriting_chains_fictionalqa_answer_f1.csv
  python3 scripts/fictionalqa/bleurt_eval_fictionalqa.py --smoke-test
"""
...
RESULTS_DIR = REPO_ROOT / "results" / "fictionalqa"

DEFAULT_CHAINS_CSV = RESULTS_DIR / "rewriting_chains_fictionalqa.csv"
DEFAULT_F1_CSV = RESULTS_DIR / "rewriting_chains_fictionalqa_answer_f1.csv"
DEFAULT_OUTPUT = RESULTS_DIR / "rewriting_chains_fictionalqa_bleurt.csv"
...
def load_bleurt(checkpoint: str):
    try:
        ...
        raise ImportError(
            ...
            "Then: pip install git+https://github.com/google-research/bleurt.git"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="BLEURT evaluation on FictionalQA rewriting chains.")
    parser.add_argument("--input", type=Path, default=DEFAULT_CHAINS_CSV)
    parser.add_argument("--f1-csv", type=Path, default=None,
                        help="Answer F1 CSV for bleurt_answer mode.")
    ...
    parser.add_argument("--smoke-test", action="store_true", help="Run only on the first qid, run 0.")
    ...
    if f1_csv_path is None and not args.no_answer:
        ...
        if f1_csv_path is None:
            print("[info] F1 CSV not found at default path, skipping bleurt_answer. Pass --f1-csv to enable.")

    do_text = not (args.no_baseline and args.no_consecutive)
    do_answer = not args.no_answer and f1_csv_path is not None

    print("=" * 70)
    print(f"BLEURT eval (FictionalQA) — checkpoint={args.checkpoint}, batch={args.batch_size}")
    ...
    if args.smoke_test:
        ...
        print(f"*** SMOKE TEST: {len(df)} rows for qid={first_qid} ***")
    ...
    if args.output.exists() and not args.smoke_test:
        ...
        print(f"  already done: {len(done_keys)} rows — remaining: {len(target)}")

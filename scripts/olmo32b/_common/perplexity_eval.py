"""Perplexity evaluation under OLMo-3.1-32B-Instruct (bf16) — dataset-agnostic.

Sliding-window PPL of each E_t, same logic as scripts/_common/perplexity_eval.py
but pinned to the OLMo-3.1-32B rewriter so PPL reflects "naturalness under
the rewriter itself" (consistent with the analysis convention).
...
from _common.olmo_model import hf_login_if_token, load_olmo  # noqa: E402
...
def main():
    ap = argparse.ArgumentParser(description="Perplexity (OLMo-3.1-32B) on rewriting chains.")
    ap.add_argument("--input", type=Path, required=True)
    ap.add_argument("--output", type=Path, default=None)
    ap.add_argument("--model", default=OLMO_MODEL_ID)
    ap.add_argument("--max-length", type=int, default=2048)
    ap.add_argument("--stride", type=int, default=1024)
    ap.add_argument("--use-4bit", action="store_true", default=False,
                    help="Optional 4-bit NF4 load (slower; default bf16).")
    ...
    tokenizer, model = load_olmo(args.model, use_4bit=args.use_4bit, padding_side="right")

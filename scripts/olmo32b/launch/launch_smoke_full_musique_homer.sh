...
#
# OLMo-3.1-32B-Instruct is served via vLLM (bf16, TP=2) for rewriting + F1.
# Perplexity uses HF transformers (vLLM doesn't expose log-likelihoods);
# OFS keeps OLMo-2-7B AFG + Gemma-3-4B AFV on HF transformers.
#
# Output goes to results/olmo32b/musique/smoke/, separate from the full-run
...
#
# Skip slow steps:
#   SKIP_PPL=1 bash launch_smoke_full_musique_homer.sh   # skip Perplexity
...
OUT_DIR="results/olmo32b/musique/smoke"
mkdir -p "$OUT_DIR"
...
echo ""
echo "############################################################"
echo "#  1/6 — Rewriting chains (OLMo-3.1-32B bf16 via vLLM)     #"
echo "############################################################"
...
echo ""
echo "############################################################"
echo "#  2/6 — Answer F1 (OLMo-3.1-32B bf16 via vLLM)            #"
echo "############################################################"
...
if [ -z "${SKIP_PPL:-}" ]; then
  echo ""
  echo "############################################################"
  echo "#  5/6 — Perplexity (OLMo-3.1-32B bf16 via HF transformers)#"
  echo "############################################################"
  echo "  (skip with SKIP_PPL=1 — slow on consumer GPUs)"
...
if [ -z "${SKIP_OFS:-}" ]; then
  ...
  echo "############################################################"
  # --limit 12 → score the first 12 step>0 rows of the chain set. Bump/remove
  # to score the full chain.
